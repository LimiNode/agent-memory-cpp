...
The continuous projection scorer is a diagnostic upper-quality scorer; exact
E5 within the Hamming shortlist is the oracle ceiling. Neither is a
document-payload or serving-latency claim: they read retained continuous
projections or original E5 embeddings for the already selected Hamming
shortlist. All comparisons use the same E5-oracle top-10 survival funnel from
raw MIH union through Hamming and K2.
...
projection scorer empirically matches the exact-E5-within-Hamming oracle
ceiling in this slice, but it is not itself a mathematical ceiling; the table
therefore labels the exact-E5 value explicitly.

| MIH candidate target | K2 | Binary ADC | Oracle ceiling (exact E5) | Ceiling - ADC |
| ---: | ---: | ---: | ---: | ---: |
...
The K2=256 gap is also small across every tested Hamming shortlist, rather
than only in the K1=768 slice:

| MIH candidate target | K1=512 | K1=768 | K1=1024 | K1=1536 |
| ---: | ---: | ---: | ---: | ---: |
| 8,192 | +0.000319 | +0.000591 | +0.000990 | +0.001310 |
| 12,288 | +0.000383 | +0.000990 | +0.001390 | +0.001981 |
| 16,384 | +0.000415 | +0.001006 | +0.001518 | +0.002109 |

At the practical K2=256 operating point, binary ADC is therefore close to the
exact-E5-within-Hamming oracle ceiling: the observed gap is 0.000591--0.001006
in the K1=768 slice and remains at most 0.002109 in the full tested K1 grid.
This does not support broadly replacing ADC. At tight K2=64 the gap is
materially larger (0.012524--0.017380), with K2=128 between those regimes. A
bounded asymmetric continuous or residual scorer is consequently a targeted
follow-up for tight second-stage budgets, rather than a replacement for the
existing K2=256 path.
...
The continuous row is a diagnostic upper-quality scorer and the exact row is
the oracle ceiling; retained continuous document projections and original E5
vectors are not the compact serving representation. The study measures oracle
survival, not production latency or payload. The next branch should first
measure the native cost of K2=64/128/256, then decide whether a bounded
residual or asymmetric continuous scorer can recover a useful fraction of the
K2=64/128 gap without invalidating the compact binary pipeline.
