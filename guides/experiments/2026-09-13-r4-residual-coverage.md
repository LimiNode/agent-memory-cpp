...
with deep support covers `.9961`.  This combined value is computed from the
deep route's address-rank membership (not its 100k budget miss list), so it is
not confused with the budgeted `.9796 @ 50k` union result.  Thus the measured
