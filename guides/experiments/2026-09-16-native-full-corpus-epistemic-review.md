...
| packed THQ4 implementation is ordered correctly | exhaustive byte-LUT self-test plus independent coordinate-reference top-128 parity `8/8` | native scalar THQ ordering is correct on the eight-query control |
...
but it does not reject THQ as a candidate prefilter.  The unresolved decision
is the native four-arm R4 candidate gate, because its frozen candidate payload
is not present in this worktree.  The native executable now has a fail-closed
