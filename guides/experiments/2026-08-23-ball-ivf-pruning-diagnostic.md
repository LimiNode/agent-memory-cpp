...
subtraction before the `max(0, ...)` clamp and loads the byte-identified
BinaryIVF artifacts from the preceding calibration run; it does not retrain
codebooks. At the most relevant K=768 cutoff, 1024 lists prune exactly 0% of
document mass; 4096 lists prune only 0.97%. Even at K=10, the 4096-list case
removes only 2.36% of documents.
