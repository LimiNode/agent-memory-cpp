...
TranslationMetaComponent
  provider/model/package digest, detected source language, pivot path,
  source revision and quality hint
```
...
enum class TranslationMode : uint8_t { Disabled, OfflineAdapter, ExternalService };
...
struct TranslationPolicy {
    ...
    bool store_translated_projection = false;
};
...
struct MemoryProfileSpec {
    ...
    bool enable_translation_projection = false;
...
| TranslationProjection=true требует translation_policy.mode != Disabled, non-empty canonical_language и store_translated_projection=true | "TranslationProjection requires active TranslationPolicy" |
