...
def route(root,record,documents):
    rr=root/f"seed-{int(record['seed'])}"; m={x['role']:x for x in record['mappings']}; off=np.fromfile(rr/m['address_offsets']['file'],dtype='<u4'); cnt=np.fromfile(rr/m['address_counts']['file'],dtype='<u4'); phys=np.fromfile(rr/m['physical_to_document']['file'],dtype='<i4'); req(int(cnt.sum())==len(phys),'route size')
...
def main():
 ...
 with flat.open('wb') as ff, blocked.open('wb') as bf:
  for qi in range(QUERIES):
   ...
   arr=np.asarray(selected[:BUDGET],dtype=np.int32); payload=np.empty((len(arr),RECORD),dtype=np.uint8); payload[:,:4]=arr.view(np.uint8).reshape(len(arr),4); payload[:,4:]=np.asarray(codes[arr]); data=payload.tobytes(); ff.write(data); flat_sha.update(data); pad=(-len(data))%PAGE
