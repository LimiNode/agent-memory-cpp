...
def main():
    ...
    result = {"schema_version": 1, "family": "cosine_lsh_margin_multiprobe_oracle_v1", "documents": n, "queries": q, "dimension": d, "tables": args.tables, "bits_per_table": args.bits, "seeds": [int(v) for v in args.seeds.split(",")], "probe_policy": "exact_bucket_plus_low_margin_single_bit_flips", "rows": rows, "interpretation_status": "MARGIN_MULTIPROBE_ORACLE_NOT_PHYSICAL_INDEX", "production_activation": False}; args.output.parent.mkdir(parents=True, exist_ok=True); args.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
