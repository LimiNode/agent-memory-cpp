...
def main():
    ...
    audit = {"schema_version": 5, "family": "thq_faiss_lsq_replay_audit_v2", "status": "PASS", "source_replay": True, "persisted_code_decode_replay": True, "centroid_source_replay": True, "result_sha256": sha256(a.result), "runner_sha256": sha256(a.runner), "input_hashes": source_hashes, "artifact_hashes": {"models": sha256(a.models), "codes": sha256(a.codes)}, "query_count": QUERY_COUNT, "payloads": list(payloads), "lsq_seed_base": int(result["lsq_seed_base"]), "effective_seed_by_payload": result["effective_seed_by_payload"], "candidate_union_documents": int(result["candidate_union_documents"]), "fit_seconds_by_payload": result["fit_seconds_by_payload"], "candidate_union_encode_seconds_by_payload": result["candidate_union_encode_seconds_by_payload"], "candidate_union_encode_docs_per_second_by_payload": result["candidate_union_encode_docs_per_second_by_payload"], "faiss_omp_threads": int(result["faiss_omp_threads"]), "row_count": len(rows), "unique_query_arm_count": len(row_map), "summaries": summaries, "checks": ["source/result/artifact SHA binding", "explicit effective Faiss seed provenance", "source-derived 25k THQ centroid equality", "independent THQ top128 replay", "independent LSQ subcodebook decode without Faiss", "cosine top10, overlap, teacher and nDCG replay", "final-norm sidecar, timing, union cardinality and logical storage replay", "summary replay"], "limitations": ["Faiss fit/assignment is hash-bound, not independently retrained", "research LSQ control, not AVQ/AAQ/QINCo", "candidate-local side-code assignment benchmark; held-out confirmation pending"]}
