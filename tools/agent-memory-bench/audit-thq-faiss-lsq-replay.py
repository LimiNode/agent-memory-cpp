...
def main():
    ...
    result = json.loads(a.result.read_text(encoding="utf-8")); require(result.get("family") == "thq_faiss_lsq_replay_v2" and result.get("status") == "EXECUTED" and result.get("source_replay") is True, "result is not LSQ source-replay evidence"); require(result.get("runner_sha256") == sha256(a.runner), "runner SHA differs"); require(result.get("query_count") == QUERY_COUNT and result.get("metric") == "cosine" and result.get("faiss_version") == "1.15.0", "LSQ protocol differs"); config = result.get("lsq_config", {}); require(set(config) == {"train_iters", "train_ils_iters", "encode_ils_iters", "icm_iters", "nperts", "random_seed", "train_rows", "base_train_rows"}, "LSQ effective configuration is missing"); require(all(int(config[key]) > 0 for key in config), "LSQ effective configuration is not positive")
    ...
    audit = {"schema_version": 3, "family": "thq_faiss_lsq_replay_audit_v2", "status": "PASS", "source_replay": True, "persisted_code_decode_replay": True, "centroid_source_replay": True, "result_sha256": sha256(a.result), "runner_sha256": sha256(a.runner), "input_hashes": source_hashes, "artifact_hashes": {"models": sha256(a.models), "codes": sha256(a.codes)}, "query_count": QUERY_COUNT, "row_count": len(rows), "unique_query_arm_count": len(row_map), "summaries": summaries, "checks": ["source/result/artifact SHA binding", "source-derived 25k THQ centroid equality", "independent THQ top128 replay", "independent LSQ subcodebook decode without Faiss", "cosine top10, overlap, teacher and nDCG replay", "final-norm sidecar and logical storage replay", "summary replay"], "limitations": ["Faiss fit/assignment is hash-bound, not independently retrained", "research LSQ control, not AVQ/AAQ/QINCo", "candidate-local side-code replay; held-out confirmation pending"]}
