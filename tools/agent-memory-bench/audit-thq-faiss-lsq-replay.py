...
def main():
    ...
    docs = np.memmap(a.documents, mode="r", dtype="<f4", shape=(1_000_000, D)); train_n = a.train_vectors.stat().st_size // (D * 4); train_rows = int(result.get("lsq_train_rows", config["train_rows"])); require(256 <= train_rows <= train_n, "LSQ train row count differs"); train = np.asarray(np.memmap(a.train_vectors, mode="r", dtype="<f4", shape=(train_n, D))[:train_rows], dtype=np.float32); queries = np.memmap(a.queries, mode="r", dtype="<f4", shape=(QUERY_COUNT, D)); qids = np.memmap(a.qrel_ids, mode="r", dtype="<i8", shape=(QUERY_COUNT, 20)); qscores = np.memmap(a.qrel_scores, mode="r", dtype="<f4", shape=(QUERY_COUNT, 20)); teacher = np.memmap(a.teacher_ids, mode="r", dtype="<i8", shape=(QUERY_COUNT, 10)); thresholds = np.fromfile(a.thq4_thresholds, dtype="<f4").reshape(D, 3); thq = np.memmap(a.thq4_codes, mode="r", dtype=np.uint8, shape=(1_000_000, THQ_BYTES))
