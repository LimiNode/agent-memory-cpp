...
def validate_bootstraps(bootstrap_dir: Path, contribution_paths: dict[str, Path], identity: dict[str, Any]) -> list[dict[str, Any]]:
...
def write_manifest(args: Any) -> None:
    ...
    comparisons = validate_bootstraps(args.bootstrap_dir, paths, identity)
    matrix = json_object(args.matrix)
    require(matrix.get("schema_version") == 1 and matrix.get("family") == "mih_budgeted_confidence_k1_matrix_v1", "matrix identity is invalid")
    manifest = {
        "schema_version": 1,
        "family": "mih_budgeted_confidence_evidence_v1",
        "matrix_sha256": sha256_file(args.matrix),
        "evaluation_identity": identity,
        "common_contract": {"evaluator_source_files_sha256": contract[0], "evaluator_source_bundle_sha256": contract[1], "evaluator_runtime": contract[2], "calibration_materialization_manifest_sha256": contract[3], "evaluation_materialization_manifest_sha256": contract[4], "calibration_train_ids_sha256": contract[5]},
...
def write_bundle(args: Any) -> None:
    ...
    bundle_root = hashlib.sha256(json.dumps(bundle_entries, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
    bundle_manifest = {"schema_version": 1, "family": "mih_budgeted_confidence_bundle_v1", "bundle_root_sha256": bundle_root, "entries": bundle_entries}
