#!/usr/bin/env python3
"""Run the predeclared calibrated weighted-Hamming MIH comparison matrix."""
...
EXPECTED_EVALUATION = {
    ...
    "soft_candidate_target", "hamming_limit", "second_limit", "second_stage",
    "oracle_k", "candidate_limit", "itq_iterations", "itq_seeds", "hamming_policies",
}
POLICIES = ("uniform", "calibrated-centroid-separation")
...
def load_matrix(path: Path) -> dict[str, Any]:
    ...
    require(value["schema_version"] == 1 and value["family"] == "mih_calibrated_weighted_hamming_matrix_v1", "matrix identity is invalid")
...
def rows(matrix: dict[str, Any]) -> list[tuple[str, dict[str, int | str]]]:
    ...
    require(
        ...
        evaluation["soft_candidate_target"] == 12288 and evaluation["hamming_limit"] == 768 and
        ...
        evaluation["itq_iterations"] == 50 and tuple(evaluation["itq_seeds"]) == SEEDS and
        tuple(evaluation["hamming_policies"]) == POLICIES,
        "matrix contract is invalid",
    )
    result = [
        (f"mih256-confidence-target12288-h768-adc256-{policy}-seed{seed}", {"hamming_policy": policy, "seed": seed})
        for policy in POLICIES for seed in SEEDS
    ]
    require(len(result) == 10 and len({name for name, _ in result}) == 10, "matrix row expansion is invalid")
    return result
...
def run(args: Any) -> None:
    ...
    def execute(index: int, name: str, row: dict[str, int | str]) -> None:
        ...
        command = [
            ...
            "--probe-policy", "budgeted-confidence", "--soft-candidate-target", "12288",
            "--hamming-policy", str(row["hamming_policy"]), "--seed", str(row["seed"]),
            "--itq-iterations", "50", "--candidate-limit", "512", "--hamming-limit", "768",
            "--second-limit", "256", "--second-stage", "binary-adc", "--oracle-k", "10",
        ]
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                raise ValueError("incomplete policy grid was accepted")
    except (OSError, ValueError, json.JSONDecodeError) as error:
        print(f"run-mih-calibrated-weighted-hamming-matrix self-test failed: {error}", file=sys.stderr)
        return 1
    print("MIH calibrated weighted-Hamming matrix self-test passed")
    return 0
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        print(f"run-mih-calibrated-weighted-hamming-matrix: {error}", file=sys.stderr)
        return 1
    return 0
