...
def evaluate_ternary(args: Any) -> None:
    ...
    if sample_count <= 0 or sample_count > len(calibration["train_ids"]):
        raise EvaluationError("scalar projection training sample count is invalid")
    indices = numpy.arange(len(calibration["train_ids"])) if sample_count == len(calibration["train_ids"]) else numpy.sort(numpy.random.default_rng(args.seed).choice(len(calibration["train_ids"]), size=sample_count, replace=False))
    training_values = numpy.asarray(calibration["train"], dtype=numpy.float32)[indices]
    training_ids = [calibration["train_ids"][index] for index in indices]
    weights = itq_weights(training_values, args.coordinate_count, args.seed, args.itq_iterations) if args.projection == "itq" else pca_weights(training_values, args.coordinate_count)
    calibration_projection = numpy.clip(training_values, -1.0, 1.0) @ weights.T
    document_projection = numpy.clip(data["documents"], -1.0, 1.0) @ weights.T
    if args.quantizer == "binary":
        thresholds = binary_thresholds(training_values, weights)
    ...
    projection_bytes = int(numpy.asarray(weights, dtype=numpy.float32).nbytes)
    centroid_bytes = int(numpy.asarray(centers, dtype=numpy.float32).nbytes)
    write_result(args.output, {"schema_version": 2, "family": "scalar_projection_reference_v2", "projection": args.projection, "quantizer": args.quantizer, "code_assignment": code_assignment, "scoring": scoring, "symbol_count": symbol_count, "coordinate_count": args.coordinate_count, "packed_payload_bytes_per_document": payload_bytes, "evaluation_materialization_manifest_sha256": data["manifest_sha256"], "evaluation_qrels_sha256": data["evaluation_qrels_sha256"], "calibration_materialization_manifest_sha256": calibration["manifest_sha256"], "training_sample_count": sample_count, "training_sample_ids_sha256": canonical_ids_sha256(training_ids), "projection_weights_sha256": hashlib.sha256(numpy.asarray(weights, dtype="<f4").tobytes()).hexdigest(), "centroids_sha256": hashlib.sha256(numpy.asarray(centers, dtype="<f4").tobytes()).hexdigest(), "projection_bytes": projection_bytes, "centroid_bytes": centroid_bytes, "total_model_bytes": projection_bytes + centroid_bytes, "seed": args.seed, "itq_iterations": args.itq_iterations if args.projection == "itq" else 0, "kmeans_iterations": args.kmeans_iterations if args.quantizer == "kmeans" else 0, "oracle_k": args.oracle_k, "candidate_limit": args.candidate_limit, "symbol_frequencies": symbol_frequencies(document_codes, symbol_count), "total_marginal_symbol_entropy_bits": total_entropy, "maximum_marginal_symbol_entropy_bits": maximum_entropy, "normalized_marginal_symbol_entropy": total_entropy / maximum_entropy, **metrics}, contributions, args.contributions_output, identity, data["query_ids"])
