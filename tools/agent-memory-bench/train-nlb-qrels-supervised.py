...
def train(*, materialization_root: Path, output_root: Path, bit_count: int, seed: int, epochs: int, batch_size: int, learning_rate: float, margin: float, validation_fraction: float, hard_negative_count: int, consumed_negatives_per_query: int, initialization_mode: str, itq_iterations: int, torch_threads: int, triplet_weight: float, reconstruction_weight: float, decorrelation_weight: float, row_orthogonality_weight: float) -> dict[str, Any]:
    ...
    if triplet_weight < 0.0 or not math.isfinite(triplet_weight):
        raise SupervisedTrainingError("triplet weight must be finite and nonnegative")
    base = load_base()
    try:
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, len(order), batch_size):
            ...
            loss = triplet_weight * triplet + reconstruction_weight * reconstruction + decorrelation_weight * decorrelation + row_orthogonality_weight * orthogonality
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--torch-threads", type=int, default=18); parser.add_argument("--triplet-weight", type=float, default=1.0); parser.add_argument("--reconstruction-weight", type=float, default=0.01)
