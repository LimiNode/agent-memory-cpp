...
def main() -> None:
    ...
    codes=raw[:,:384].astype(np.float32)
    scales=np.frombuffer(np.asarray(raw[:,384:388], dtype=np.uint8).tobytes(),
                         dtype="<f4").copy()
    vectors=(codes-127.0)*scales[:,None]
    rows=vectors.reshape(count//16,16,384); rows=rows[:count//16*16]
