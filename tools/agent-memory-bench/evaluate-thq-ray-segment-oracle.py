    ...
    for qi in range(start, start+count):
        ...
        for anchor in targets[qi,:max(1,a.anchors)]:
            ...
            for b in range(0,n,a.block_size):
                ...
                alpha=np.clip((diff@v)/vv,0.0,1.0)
                score[b:e]=d2-alpha*alpha*vv
        ...
        rows.append(out)
    result={'schema_version':1,'family':'thq_ray_segment_geometry_oracle_v1','queries':count,'query_start':start,'anchors':a.anchors,'top_budget':a.top_budget,'rows':rows,'elapsed_seconds':time.perf_counter()-begun}
