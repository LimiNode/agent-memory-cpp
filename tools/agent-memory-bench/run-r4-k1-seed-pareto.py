...
def bind_file(path: Path, role: str) -> dict[str, Any]:
...
def main() -> None:
    ...
    routes={}; inverse={}; orders={}; bindings=[]; input_bindings=[]
    teacher_path=Path(thq['references']['teacher_ids']['path'])
    input_bindings.append(bind_file(teacher_path, 'teacher_ids'))
    for rec in layout['seeds']:
        ...
        for mapping in rec['mappings']:
            input_bindings.append(bind_file(a.r4_layout_root/f"seed-{seed}"/mapping['file'],
                                            f"seed-{seed}:{mapping['role']}"))
    ...
    receipt={'schema_version':1,'family':raw['family'],'execution_status':'EXECUTED','production_activation':False,'summaries':summaries,
             ...
             'native_bindings':bindings,'input_bindings':input_bindings,'runner_sha256':sha256(Path(__file__)),'raw_output':{'path':str(a.raw_output),'bytes':len(payload),'sha256':hashlib.sha256(payload).hexdigest(),'rows':len(rows)}}
