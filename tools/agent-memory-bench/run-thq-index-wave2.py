...
def compose_imi_cell(codes):
 a=codes[:,:3]; b=codes[:,3:6]
 ca=(a[:,0].astype(np.int32)*16+a[:,1].astype(np.int32)*4+a[:,2].astype(np.int32))
 cb=(b[:,0].astype(np.int32)*16+b[:,1].astype(np.int32)*4+b[:,2].astype(np.int32))
 if int(ca.max()) > 63 or int(cb.max()) > 63: raise AssertionError('IMI subspace out of range')
 cells=ca*64+cb
 if int(cells.min()) < 0 or int(cells.max()) >= 4096: raise AssertionError('IMI cell out of range')
 return cells
...
def main():
 ...
 imi_cell=compose_imi_cell(codes)
 ...
 for qi in range(qn):
  ...
  sr=[]; ir=[]
  for L in budgets:
   spann_L=min(L,len(postings)); scells=spann_order[:spann_L]; icells=imi_order[:L]; sc=np.concatenate([postings[c] for c in scells if postings[c].size]) if any(postings[c].size for c in scells) else np.empty(0,dtype=np.int64); ic=np.concatenate([imi_post[c] for c in icells if imi_post[c].size]) if any(imi_post[c].size for c in icells) else np.empty(0,dtype=np.int64); sr.append({'L':L,'effective_L':spann_L,'postings_touched':spann_L,'nonempty_postings_touched':int(sum(postings[c].size>0 for c in scells)),'empty_posting_fraction':float(sum(postings[c].size==0 for c in scells)/spann_L),'posting_entries_touched':int(sc.size),'candidate_docs':int(sc.size),'teacher_recall':float(np.isin(teachers[qi],sc).sum()/10)}); ir.append({'L':L,'effective_L':L,'cells_touched':L,'nonempty_cells_touched':int(sum(imi_post[c].size>0 for c in icells)),'empty_cell_fraction':float(sum(imi_post[c].size==0 for c in icells)/L),'posting_entries_touched':int(ic.size),'candidate_docs':int(ic.size),'teacher_recall':float(np.isin(teachers[qi],ic).sum()/10)})
