...
def main():
 ...
 tq=np.load(a.tq_payload,allow_pickle=False); unique=np.asarray(tq['document_ids'],np.int64); base=np.asarray(tq['base'],np.float32); decoded=np.asarray(tq['decoded1'],np.float32); row_ids=np.asarray(tq['row_ids'],np.int64); row_off=np.asarray(tq['row_offsets'],np.int64); pos={int(d):i for i,d in enumerate(unique)}; residual=np.asarray(docs[unique],np.float32)-base-decoded; residual_norms=np.linalg.norm(residual.astype(np.float64),axis=1).astype(np.float32); source_norms=np.linalg.norm(np.asarray(docs[unique],np.float32).astype(np.float64),axis=1).astype(np.float32)
 arms=[]; projections={}
 for m in (32,64,128,256,384):
 ...
 for qi,q in enumerate(queries):
  ids=row_ids[row_off[qi]:row_off[qi+1]]; idx=np.asarray([pos[int(d)] for d in ids]); b=base[idx]+decoded[idx]; qn=max(float(np.linalg.norm(q)),1e-30); bnorm=np.maximum(np.linalg.norm(b,axis=1),1e-30); exact=np.asarray(docs[ids],np.float64); exact_scores=(exact@np.asarray(q,dtype=np.float64))/qn; tq_scores=(b@q)/(bnorm*qn); order=np.lexsort((ids,-tq_scores)); ranked=ids[order]
  for m in (32,64,128,256,384):
   for dist in ('gaussian','rademacher'):
    name=f'qjl_{dist}{m}'; proj=projections[name]; signs,_=qjl.encode(residual[idx],proj); est=qjl.estimate_dot_reference(q,signs,residual_norms[idx],proj) if dist=='gaussian' else qjl.estimate_dot_rademacher_control(q,signs,residual_norms[idx],proj); score=(b@q+est)/(qn); merged=ids[np.lexsort((ids,-score))]; side=m//8+4; rows.append({'query':qi,'arm':name,'top10_ids':merged[:10].astype(int).tolist(),'thq4_top128_ids':ids.astype(int).tolist(),'qrels_ndcg10':ndcg(merged,qids[qi],grades[qi]),'teacher_overlap':float(np.isin(teacher[qi],merged[:10]).sum()/10),'side_payload_bytes':side,'cascade_total_bytes':96+52+side,'bytes_touched_this_query':TOP*(52+side),'qjl_denominator':'unit_norm_contract_constant_1','global_model_bytes':m*D*4,'mean_abs_score_error':float(np.mean(np.abs(score-exact_scores)))})
 ...
 summaries={arm:{'mean_qrels_ndcg10':float(np.mean([r['qrels_ndcg10'] for r in rows if r['arm']==arm])),'p05_qrels_ndcg10':float(np.percentile([r['qrels_ndcg10'] for r in rows if r['arm']==arm],5)),'worst_qrels_ndcg10':float(np.min([r['qrels_ndcg10'] for r in rows if r['arm']==arm])),'mean_teacher_overlap':float(np.mean([r['teacher_overlap'] for r in rows if r['arm']==arm])),'side_payload_bytes':next(r['side_payload_bytes'] for r in rows if r['arm']==arm),'cascade_total_bytes':next(r['cascade_total_bytes'] for r in rows if r['arm']==arm),'global_model_bytes':next(r.get('global_model_bytes',0) for r in rows if r['arm']==arm),'qjl_denominator':next(r['qjl_denominator'] for r in rows if r['arm']==arm)} for arm in sorted({r['arm'] for r in rows})}
 a.artifact.parent.mkdir(parents=True,exist_ok=True); np.savez_compressed(a.artifact,document_ids=unique,base=base,decoded1=decoded,residual_norms=residual_norms,source_norms=source_norms,**{n:np.asarray(p.matrix,dtype='<f4') for n,p in projections.items()},**{f'{n}_signs':np.asarray(qjl.encode(residual, p)[0],dtype=np.uint8) for n,p in projections.items()}); src={n:getattr(a,n.replace('-','_')) for n in ('documents','queries','qrel-ids','qrel-scores','teacher-ids','thq4-codes','thq4-thresholds','candidate-flat','candidate-raw','candidate-receipt','tq-payload')}; out={'schema_version':2,'family':'thq_qjl_residual_gate_v2','status':'EXECUTED','source_replay':True,'metric':'cosine','query_count':Q,'projection_rows':[32,64,128,256,384],'distributions':['gaussian','rademacher'],'denominator_contract':'unit_norm_constant_1','global_model_bytes_by_width':{str(m):m*D*4 for m in (32,64,128,256,384)},'source_hashes':{n:sha256(v) for n,v in src.items()},'runner_sha256':sha256(Path(__file__)),'artifact_sha256':sha256(a.artifact),'summaries':summaries,'rows':rows,'limitations':['candidate-local canonical THQ top128 shell','QJL is a score correction, not a vector decoder','source norm is retained as a numerical diagnostic; serving denominator is the normalized-E5 constant 1.0','exact corrected norm is not a separate arm']}; a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
