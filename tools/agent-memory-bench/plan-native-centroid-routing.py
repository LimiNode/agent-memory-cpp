...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    require(value.get("evaluation") == {"query_count": 648, "target_candidate_fractions": [.05, .10, .25], "matched_mass_rule": "ranked_centroid_lists_until_target_document_count_v1", "float_oracle": "exact_fp32_inner_product_centroid_order_descending_then_centroid_id_ascending_v1", "binary_shortlist_rule": "ceil(shortlist_multiplier*target_candidate_fraction*centroid_count)_then_exact_fp32_rerank_v1", "hnsw_shortlist_rule": "ceil(2*target_candidate_fraction*centroid_count)_then_exact_fp32_rerank_v1"}, "native centroid routing evaluation differs")
    ...
    require(value.get("hnsw") == {"connectivity": [8, 16], "ef_construction": 200, "ef_search": [256, 512, 2048, 8192], "seed": 20260825, "pinned_revision": "3f3429661187e4c24a490a0f148fc6bc89042b3d"} and value.get("timing") == {"warmup_repeats": 2, "measured_repeats": 7, "concurrency": "single_native_routing_thread_v1", "scope": "warm_in_memory_native_centroid_routing_only_excludes_query_embedding_document_cascade_and_index_build_v1"}, "native centroid routing timing matrix differs")
    ...
    for scale in contract["scales"]:
        for centroid_count in scale["centroid_counts"]:
            for fraction in contract["evaluation"]["target_candidate_fractions"]:
                rows.extend({"scale": scale["id"], "centroid_count": centroid_count, "target_candidate_fraction": fraction, "treatment": treatment, "shortlist_multiplier": multiplier if treatment.startswith("rademacher") else (2 if treatment.startswith("hnsw") else None), "hnsw_connectivity": connectivity if treatment.startswith("hnsw") else None, "hnsw_ef_search": ef_search if treatment.startswith("hnsw") else None} for treatment in contract["routing_treatments"] for multiplier in (contract["shortlist_multipliers"] if treatment.startswith("rademacher") else [None]) for connectivity in (contract["hnsw"]["connectivity"] if treatment.startswith("hnsw") else [None]) for ef_search in (contract["hnsw"]["ef_search"] if treatment.startswith("hnsw") else [None]))
...
def main() -> int:
    ...
    try:
        ...
        require(result["row_count"] == 180, "native centroid routing row count differs")
