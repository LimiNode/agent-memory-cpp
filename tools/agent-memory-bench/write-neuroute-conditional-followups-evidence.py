...
def run(a):
 ...
 roots=authoritative.validate_roots([("de-25k",a.de_e5_root),("fr-25k",a.fr_e5_root),("ja-25k",a.ja_e5_root),("de-1m",a.de_1m_e5_root)])
 ...
 require(authoritative.validate_roots([("de-25k",a.de_e5_root),("fr-25k",a.fr_e5_root),("ja-25k",a.ja_e5_root),("de-1m",a.de_1m_e5_root)])==roots,"conditional authoritative roots changed during replay")
 receipt={"schema_version":1,"family":"neuroute_conditional_representation_evidence","passed":True,"contract_sha256":r.sha256(a.contract),"result_sha256":r.sha256(a.result),"source_files_sha256":r.source_hashes(),"writer_sha256":r.sha256(Path(__file__)),"authoritative_qrels_validator_sha256":r.sha256(THIS/"neuroute_authoritative_qrels.py"),"authoritative_roots":roots,"quality_replay_byte_identical":True,"authoritative_qrels_to_quality_replay_passed":True,"decision":result["decision"]}; a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_bytes(r.canonical(receipt))
def main():
