#!/usr/bin/env python3
"""Fail-closed audit for the representative-max-anchor route probe."""
...
def main() -> None:
    ...
    require(receipt["family"] == "semantic_r4_representative_max_anchor_route_v1", "wrong family")
    ...
    arms = receipt["arms"]; require(len(arms) == 4, "arm count mismatch")
    for arm in arms:
    ...
    print(json.dumps({"status": "PASS", "family": receipt["family"], "rows": len(rows), "arms": arms}, sort_keys=True))
