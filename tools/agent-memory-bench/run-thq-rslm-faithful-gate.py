...
def main() -> None:
    ...
    for bits in (2, 3, 4):
        ...
        models[f"rslm{bits}-faithful"] = {"bits": bits, "decoded": faithful_decoded, "full_vector": True, "side_bytes": int((D * bits + 7) // 8 + 4), "source": "official-relative-mode", "artifact_sha256": sha256(faithful_path)}
        ...
        local_decoded.flush()
        models[f"rslm{bits}-local"] = {"bits": bits, "decoded": local_decoded, "full_vector": False, "side_bytes": int((D * bits + 7) // 8), "source": "historical-local-control", "artifact_sha256": sha256(local_path)}
    rows = []
    for qi, query in enumerate(queries):
        ...
        for name, model in models.items():
            ...
            rows.append({"query": qi, "arm": name, "top10_ids": selected.astype(int).tolist(), "candidate_fp32_overlap": float(np.isin(exact, selected).sum() / 10.0), "teacher_overlap": float(np.isin(teacher_ids[qi], selected).sum() / 10.0), "qrels_ndcg10": ndcg10(selected, qrel_ids[qi], qrel_scores[qi]), "side_payload_bytes": int(model["side_bytes"]), "cascade_total_bytes": 96 + int(model["side_bytes"])})
    summaries = {}
    for name in models:
