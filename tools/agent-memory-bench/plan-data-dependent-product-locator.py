...
def load_contract(path: Path) -> dict[str, Any]:
    ...
    require(value.get("training_source") == "frozen_train_itq256_codes_derived_only_from_pinned_artifact_and_frozen_train_e5_vectors_only" and value.get("faiss_version") == "1.13.2" and value.get("implicit_cell_budgets") == [4096, 16384, 65536] and value.get("target_candidate_fractions") == [0.05, 0.10, 0.25], "data-dependent product locator matrix differs")
    ...
    require(value.get("treatments") == [{"id": "local_binary_medoids", "bit_decomposition": "contiguous_balanced_blocks_v1", "local_codebook": "train_only_hamming_kmedoids_v1"}, {"id": "permuted_binary_medoids", "bit_decomposition": "train_only_bit_permutation_balanced_blocks_v1", "local_codebook": "train_only_hamming_kmedoids_v1"}, {"id": "float_e5_product", "bit_decomposition": "contiguous_balanced_e5_dimension_blocks_v1", "local_codebook": "train_only_float_kmeans_on_frozen_e5_vectors_v1"}], "data-dependent product locator treatments differ")
    require(value.get("amends_measurement_contract_sha256") == "0ce223f825adc5b55ef5662ca628b0ddd5d5fcf8b40ea9aa2345414f26809aadb" and value.get("routing") == "best_first_sum_local_hamming_then_cell_key_lexicographic_v1" and value.get("routing_by_treatment") == {"local_binary_medoids": "best_first_sum_local_hamming_then_cell_key_lexicographic_v1", "permuted_binary_medoids": "best_first_sum_local_hamming_then_cell_key_lexicographic_v1", "float_e5_product": "best_first_sum_local_squared_l2_e5_then_cell_key_lexicographic_v1"} and value.get("training") == {"binary_hamming_kmedoids": {"sample": "deterministic_evenly_spaced_train_positions_v1", "sample_count": 1024, "iterations": 4, "initialization": "farthest_first_hamming_with_lowest_position_ties_v1"}, "float_e5_kmeans": {"clusters_per_block": 4, "iterations": 25, "restarts": 1, "seed": 20260826, "spherical": False}} and value.get("cascade") == {"hamming_limit": 768, "adc_limit": 256, "exact_limit": 256, "oracle_k": 10} and value.get("exploratory_gate") == {"candidate_fraction": 0.05, "minimum_e5_oracle_survival_after_adc": 0.70, "meaning": "below_this_gate_do_not_add_native_empty_cell_or_trie_engineering"} and value.get("confirmation") == "forbidden" and value.get("library_dependency") == "forbidden_external_research_only", "data-dependent product locator scope differs")
    return value
