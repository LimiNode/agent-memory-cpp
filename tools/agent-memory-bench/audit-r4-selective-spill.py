...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--result',type=Path,required=True); p.add_argument('--runner',type=Path,required=True); p.add_argument('--raw',type=Path,required=True); a=p.parse_args(); d=json.loads(a.result.read_text()); rows=json.loads(a.raw.read_text())['rows']; query_ids=[int(x) for x in d['query_ids']]; q=len(query_ids)
 if d.get('family')!='r4_cross_seed_replication_control_v2' or d['execution_status']!='EXECUTED' or d['production_activation'] is not False: raise SystemExit('family/execution flags invalid')
 ...
 if sorted({int(r['query']) for r in rows}) != sorted(query_ids): raise SystemExit('query IDs mismatch')
 for s in d['summary']:
