...
def main():
 ...
 factors=[1.0,1.25,1.5,2.0]; policies={'primary_only':None,'random_secondary':random_order,'address_index_boundary_proxy':boundary_order,'address_index_disagreement_proxy':complement_order}; rows=[]
 ...
 raw=(json.dumps({'schema_version':1,'rows':rows},sort_keys=True,separators=(',',':'))+'\n').encode(); a.raw_output.parent.mkdir(parents=True,exist_ok=True); a.raw_output.write_bytes(raw); out={'schema_version':1,'family':'r4_cross_seed_replication_control_v2','execution_status':'EXECUTED','production_activation':False,'fixture_manifest_sha256':sha256(a.thq_manifest),'r4_manifest_sha256':sha256(a.r4_manifest),'runner_sha256':sha256(Path(__file__)),'query_ids':query_ids,'budgets':budgets,'replication_factors':factors,'summary':summary,'raw_output':{'path':str(a.raw_output),'bytes':len(raw),'sha256':hashlib.sha256(raw).hexdigest()},'protocol':{'primary_seed':2026082701,'secondary_seed':2026082702,'assignment_teacher_free':True,'policies':['primary_only','random_secondary','address_index_boundary_proxy','address_index_disagreement_proxy'],'budget_definition':'unique document ids','query_order':'model_order for both seed streams','physical_page_bytes':'not measured','scope':'cross-seed replication control; not SPANN closure or SOAR residual assignment','payload_replication':'doc IDs only; canonical payload remains single','address_index_proxies_not_geometry':True}}; a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(out,indent=2)+'\n')
