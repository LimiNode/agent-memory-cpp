...
def row_is_complete(report_path: Path, contribution_path: Path, row: dict[str, int | str]) -> bool:
    ...
    return (
        ...
        report.get("second_limit") == 256 and report.get("second_stage") == "binary-adc" and report.get("query_count") == 1252 and
        report.get("per_query_contributions_path") == contribution_path.name and report.get("per_query_contributions_sha256") == sha256_file(contribution_path)
    )
