...
def main() -> None:
    ...
    require(args.documents.stat().st_size == 1_000_000 * D * 4, "documents must be 1M FP32x384")
    # Keep the 1.5-GB corpus memory-mapped; only the frozen THQ shell is
    # decoded below.  Materializing the whole corpus here can turn a replay
    # into a paging benchmark and is unnecessary for the candidate-local gate.
    docs = np.memmap(args.documents, mode="r", dtype="<f4", shape=(1_000_000, D))
    train = load_f32(args.train_vectors); queries = load_f32(args.queries, QUERY_COUNT); qids = np.asarray(np.memmap(args.qrel_ids, mode="r", dtype="<i8", shape=(QUERY_COUNT, 20))); grades = np.asarray(np.memmap(args.qrel_scores, mode="r", dtype="<f4", shape=(QUERY_COUNT, 20))); teacher = np.asarray(np.memmap(args.teacher_ids, mode="r", dtype="<i8", shape=(QUERY_COUNT, 10)))
    raw = json.loads(args.candidate_raw.read_text(encoding="utf-8")); counts = np.asarray([int(row["candidate_count"]) for row in raw["rows"]], dtype=np.int64); offsets = np.concatenate(([0], np.cumsum(counts))); total = int(offsets[-1]); receipt = json.loads(args.candidate_receipt.read_text(encoding="utf-8")); record_bytes = int(receipt.get("flat_file", {}).get("record_bytes", 148)); require(record_bytes in (100, 148), "unsupported candidate record size"); require(args.candidate_flat.stat().st_size == total * record_bytes, "candidate cardinality differs"); records = np.memmap(args.candidate_flat, mode="r", dtype=np.uint8, shape=(total, record_bytes)); candidate_ids = np.asarray(records[:, :4]).copy().view("<i4").reshape(-1).astype(np.int64); require(receipt.get("execution_status") == "EXECUTED" and receipt.get("raw_sha256") == sha256(args.candidate_raw) and receipt.get("flat_file", {}).get("sha256") == sha256(args.candidate_flat), "candidate provenance differs")
