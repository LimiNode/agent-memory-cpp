...
enum class Kernel {
  ...
  RawSignHamming, RawSignHammingUnrolled, RawSignHammingAvx2, RawSignDot,
...
// Raw sign control over the first 208 coordinates. This is not a trained ITQ
// transform and does not implement an ADC codebook.
...
float score_raw_sign208(const std::uint8_t* bytes, const float* query) {
  float score = 0.0f;
  for (std::size_t i = 0; i < 208; ++i) {
...
int main(int argc, char** argv) {
  ...
  const std::vector<Method> methods = {
      ...
      {"raw_sign208_hamming", 26, Kernel::RawSignHamming, true},
      {"raw_sign208_hamming_unrolled", 26, Kernel::RawSignHammingUnrolled, true},
      {"raw_sign208_hamming_avx2", 26, Kernel::RawSignHammingAvx2, true},
      {"raw_sign208_dot", 26, Kernel::RawSignDot},
      {"thq3_gaussian_threshold_control", 96, Kernel::Thq3, true}, {"thq3_gaussian_threshold_control_unrolled", 96, Kernel::Thq3Unrolled, true}, {"thq3_gaussian_threshold_control_avx2", 96, Kernel::Thq3Avx2, true},
      {"thq4_gaussian_threshold_control", 144, Kernel::Thq4, true}, {"thq4_gaussian_threshold_control_unrolled", 144, Kernel::Thq4Unrolled, true}, {"thq4_gaussian_threshold_control_avx2", 144, Kernel::Thq4Avx2, true},
      {"thq5_gaussian_threshold_control", 192, Kernel::Thq5, true}, {"thq5_gaussian_threshold_control_unrolled", 192, Kernel::Thq5Unrolled, true}, {"thq5_gaussian_threshold_control_avx2", 192, Kernel::Thq5Avx2, true}};
  ...
  for (std::size_t i = 0; i < records; ++i) {
    ...
    packed[11].push_back(encode_raw_sign208(&vectors[i * kDimensions]));
    packed[12].push_back(encode_raw_sign208(&vectors[i * kDimensions]));
  ...
  const PackedStore raw_sign_hamming = materialize_packed(packed[11]);
  const PackedStore raw_sign_dot = materialize_packed(packed[12]);
  ...
  const Packed query_raw_sign = encode_raw_sign208(query.data());
  ...
  auto score_one = [&](const Method& method, std::size_t i) {
    switch (method.kernel) {
      ...
      case Kernel::RawSignHamming: return score_hamming(raw_sign_hamming.bytes.data() + i * raw_sign_hamming.stride, query_raw_sign.bytes.data(), raw_sign_hamming.stride);
      case Kernel::RawSignHammingUnrolled: return score_hamming_unrolled(raw_sign_hamming.bytes.data() + i * raw_sign_hamming.stride, query_raw_sign.bytes.data(), raw_sign_hamming.stride);
      case Kernel::RawSignHammingAvx2:
#if AGENT_MEMORY_DOCUMENT_CODEC_HAS_AVX2
        return score_hamming_avx2(raw_sign_hamming.bytes.data() + i * raw_sign_hamming.stride, query_raw_sign.bytes.data(), raw_sign_hamming.stride);
#else
        return score_hamming(raw_sign_hamming.bytes.data() + i * raw_sign_hamming.stride, query_raw_sign.bytes.data(), raw_sign_hamming.stride);
#endif
      case Kernel::RawSignDot: return score_raw_sign208(raw_sign_dot.bytes.data() + i * raw_sign_dot.stride, query.data());
  ...
  for (std::size_t i = 0; i < std::min<std::size_t>(records, 16); ++i) {
    ...
    for (const PackedStore* store : {&raw_sign_hamming, &thq3, &thq4, &thq5}) {
      const std::size_t query_stride = store == &raw_sign_hamming ? raw_sign_hamming.stride : store->stride;
      const std::uint8_t* query_bytes = store == &raw_sign_hamming ? query_raw_sign.bytes.data() :
