...
int main(int argc, char** argv) {
  ...
  const std::vector<Method> methods = {
      ...
      {"raw_sign208_hamming", 26}, {"raw_sign208_dot", 26},
      {"thq3_gaussian_threshold_control", 96},
      {"thq4_gaussian_threshold_control", 144},
      {"thq5_gaussian_threshold_control", 192}};
  ...
  auto score_one = [&](std::size_t method, std::size_t i) {
    ...
    if (method == 11) return score_hamming(packed[11][i], query_raw_sign);
    if (method == 12) return score_raw_sign208(packed[12][i], query.data());
