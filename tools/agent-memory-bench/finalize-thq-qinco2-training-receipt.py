...
def main() -> None:
    ...
    if not epoch_matches or not mse_matches:
    ...
    validation_mse = float(mse_matches[-1])
    ...
    for index, (epoch, steps, loss) in enumerate(loss_matches):
        ...
        if index < len(mse_matches):
            trace["validation_mse"] = float(mse_matches[index])
    ...
    if checkpoint_epoch != completed_epochs:
        raise RuntimeError(f"checkpoint epoch {checkpoint_epoch} differs from log {completed_epochs}")
    ...
    if checkpoint_steps != optimizer_steps:
        raise RuntimeError(f"checkpoint optimizer steps {checkpoint_steps} differ from log {optimizer_steps}")
    checkpoint_mse = float(checkpoint.get("logger", {}).get("best_mse", float("nan")))
    if not abs(checkpoint_mse - validation_mse) <= 1e-5:
        raise RuntimeError(f"checkpoint validation MSE {checkpoint_mse} differs from log {validation_mse}")
    ...
    training.update({
        ...
        "occupancy_reset_trace": [
            {"epoch": int(epoch), "reset_count": int(count), "capacity": int(capacity),
             "per_stage_reset_count": [int(value) for value in re.findall(r"(\d+)/\d+", per_stage)]}
